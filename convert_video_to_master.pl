...
foreach my $i ( 0 .. $#streams ) {
  ...
  if ( $streams[$i]{'codec_name'} eq "rv30" ) {
  print "realmedia audio format found (rv30) \n";
  print "Attempting to convert\n";
  $realmedia = 1;
  }
} 
...
unless (fileExists($WAV_OUTPUT_FILE, "WAV")){
...
unless (fileExists($MASTER_OUTPUT_FILE, "MASTER")){
...
my $rm_convert_cmd =  MENCODER . " $video_raw  -oac pcm -ovc  lavc -vf scale= $dars{$v_aspect} -o $CONVERTED_OUTPUT_FILE ";  
