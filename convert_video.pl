...
if ( $numArgs == 0 ) {
    ...
    my $orig = "$HOMEDIR/video/upload/";
    my $new  = "$HOMEDIR/video/masters/";
    ...
    my @files = (`find $HOMEDIR/video/upload/  -type f`);
    foreach my $file (@files) {
        ...
        unless ( -e "$fileBaseName.master" ) {
            convertToMaster($file);
        }
    }

    $orig = "$HOMEDIR/video/masters/";
    $new  = "$HOMEDIR/video/public/";

    copyDirTree( "$HOMEDIR/video/masters/", "$HOMEDIR/video/public/" );

    setPath($new);

    #find all files in the master directory
    my @files = (`find $HOMEDIR/video/masters/  -name  \*.master`);
    #print "@files\n";
    foreach my $file (@files) {
...
sub copyDirTree() {
    ...
    print "input $inputDir ... o#utput $outputDir\n";
    setPath($inputDir);
